...
/**
 * Student page bootstrap.
 *
 * Include this ONCE at the very top of any student page, in the global scope
 * (not inside a function). It will:
 *   - load the database ($pdo) and the auth helpers,
 *   - start the hardened session,
 *   - enforce that the visitor is a logged-in student (redirecting to the
 *     login page and exiting if not),
 ...
 * function or method.
 *
 *   Usage from /public/student/dashboard.php:
...
// student. The role is read fresh from the database inside requireRole().
requireRole($pdo, 'student');

// Guaranteed non-null here: if the visitor weren't a valid student,
